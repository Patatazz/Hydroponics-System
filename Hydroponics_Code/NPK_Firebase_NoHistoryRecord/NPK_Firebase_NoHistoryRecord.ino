...
const char* firebaseAuth = "";  // Leave empty for now, we'll use database rules
...
void uploadToFirebase(uint16_t nitrogen, uint16_t phosphorus, uint16_t potassium) {
  ...
  // Create Firebase URL - just add the path, no .json extension needed in firebaseHost
  ...
  int httpResponseCode = http.PUT(jsonPayload);  // PUT overwrites existing data
